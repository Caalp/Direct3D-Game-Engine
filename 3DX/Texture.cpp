...
Texture::Texture(Graphics& gfx, const std::vector<std::string>& filepath, unsigned int index)
{
	...
	for (uint32_t i = 0; i < arraySize-4; i++)
	{
...
{
	std::string fullpath = std::string(TEXTURES_PATH) + std::string(filePath);
	this->bufferSlot = bufferSlot;
	HRESULT hr = D3DX11CreateShaderResourceViewFromFile(GetDevice(gfx), fullpath.c_str(), 0, 0, &srv, 0);
	if (FAILED(hr))
	{
		fullpath = filePath;
		hr = D3DX11CreateShaderResourceViewFromFile(GetDevice(gfx), fullpath.c_str(), 0, 0, &srv, 0);
		if (FAILED(hr))
		{
