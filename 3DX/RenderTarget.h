...
public:
	RenderTargetArray(Graphics& gfx, UINT w, UINT h, UINT arraySize) : RenderTarget(w,h)
	{
		...
		for (uint32_t i = 0; i < arraySize; i++)
		{
			...
			GetDevice(gfx)->CreateRenderTargetView(cubeTex.Get(), &rtvDesc, renderTargetView[i].GetAddressOf());
		}
		...
		GetDevice(gfx)->CreateShaderResourceView(cubeTex.Get(), &srvDesc, shaderResourceView.GetAddressOf());
	}
