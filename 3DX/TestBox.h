...
public:
	void CreateBoxGeometry(Graphics& gfx,Renderable& render)
	{
#pragma region CreateBox
		...
		test.Add<dvbuff::TextureCoord2D>();*/
			...
			*/
					...
					//Renderable render;
					render.indexBuffer = std::make_unique<IndexBuff>(gfx, indices);
					render.vertexBuffer = std::make_unique<VertexBuffer>(gfx, vertices);
					render.primitiveTopology = std::make_unique<PrimitiveTopology>(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
...
#pragma endregion
	...
	TestBox(Graphics& gfx, std::string sceneName) : Entity(sceneName)
	{	
		
		TransformationComponent trans;
		trans.translation = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
		trans.rotation = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
		trans.scaling = DirectX::XMFLOAT3(1.0f, 1.0f, 1.0f);
		trans.tBuffer  = std::make_shared<VSConstBuff<TransformBuffer>>();
		this->AddComponent<TransformationComponent>(trans);
		
		//CameraComponent cam;
		
		Renderable renderable;
		CreateBoxGeometry(gfx, renderable);
		...
		{
			{
				...
				s1.AddBind(std::make_shared<DrawIndexed>(0, 36u));
				s1.AddBind(std::make_shared<TransformationBuffer>(trans.tBuffer));
		...
		renderable.techniques.push_back(boxDefault);
		this->AddComponent<Renderable>(std::move(renderable));
