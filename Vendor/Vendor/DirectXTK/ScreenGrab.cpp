...
namespace
{
        ...
        ComPtr<ID3D11Texture2D>& pStaging) noexcept
    {
        ...
        {
            DirectX::DebugTrace("ERROR: ScreenGrab does not support 1D or volume textures. Consider using DirectXTex instead.\n");
        ...
        {
            DirectX::DebugTrace("WARNING: ScreenGrab does not support 2D arrays, cubemaps, or mipmaps; only the first surface is written. Consider using DirectXTex instead.\n");
        }
        ...
        if (desc.SampleDesc.Count > 1)
        {
            ...
            DXGI_FORMAT fmt = DirectX::LoaderHelpers::EnsureNotTypeless(desc.Format);
    ...
    const wchar_t* fileName) noexcept
{
    ...
    DirectX::LoaderHelpers::auto_delete_file delonfail(hFile.get());
    ...
    *reinterpret_cast<uint32_t*>(&fileHeader[0]) = DDS_MAGIC;
    ...
    hr = DirectX::LoaderHelpers::GetSurfaceInfo(desc.Width, desc.Height, desc.Format, &slicePitch, &rowPitch, &rowCount);
    if (FAILED(hr))
        return hr;
    ...
    if (DirectX::LoaderHelpers::IsCompressed(desc.Format))
    {
    ...
    bool forceSRGB)
{
    ...
    DirectX::LoaderHelpers::auto_delete_file_wic delonfail(stream, fileName);
